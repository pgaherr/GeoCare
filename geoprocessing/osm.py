...
https://github.com/CityScope/UrbanAccessAnalyzer/blob/main/UrbanAccessAnalyzer/osm.py
...
def green_areas(bounds, intersected_geom=None, min_area=200, min_width=10, buffer=5):
    ...
    green_areas_gdf = gpd.GeoDataFrame(
        {}, geometry=shapely.get_parts(green_areas_gdf), crs=crs
    )

    if intersected_geom is not None:
        intersected_geom_union = intersected_geom.to_crs(
            green_areas_gdf.crs
        ).union_all()
        geoms = list(green_areas_gdf.geometry)
        shapely.prepare(geoms)
        green_areas_gdf = green_areas_gdf[
            shapely.intersects(geoms, intersected_geom_union)
        ]
